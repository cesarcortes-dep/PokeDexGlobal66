...
| F6 | "El botón compartir debe copiar en el portapapeles el nombre del pokemon con sus atributos separados por coma" | Click copia string `nombre,attr,attr,…`; feedback visible al usuario; fallback si Clipboard API no disponible | `[x]` **Verificado en navegador:** copia `Bulbasaur,6.9 kg,0.7 m,Planta,Veneno` y el botón cambia a "¡Copiado!". Copia **lo que se ve**, no lo crudo de la API: nombre capitalizado, tipos en español y unidades incluidas. `useClipboard` con Clipboard API y fallback a `execCommand` que cubre dos casos —contexto sin HTTPS y permiso denegado—, más un `role="status"` oculto porque un cambio de texto en un botón no lo anuncia un lector de pantalla por sí solo. **El Figma no incluye este botón**: el diseño es propio, documentado como desviación forzada por el requisito |
...
| ID | Requisito (literal) | Ítems verificables | Estado |
|----|---------------------|--------------------|--------|
| D1 | "las especificaciones del diseño están en este link" — [Figma](https://www.figma.com/design/edU7Pms8bvosgSYW23yOds/Pok%C3%A9dex?node-id=0-1) | Pantallas implementadas fieles al Figma en viewport mobile | `[!]` **Implementadas con tres desviaciones documentadas.** Tokens reales del Figma (paleta de 18 tipos, Poppins, radios, bordes), tarjeta con color por tipo, chips con sus iconos, estados vacíos y detalle. Se desvía en: **(1)** dos pestañas de navegación en vez de cuatro —regiones y perfil no tienen funcionalidad detrás—; **(2)** botón de compartir, que el Figma no incluye y F6 exige; **(3)** marco de Pokédex en el detalle, decisión estética propia. Las tres son deliberadas y están razonadas |
| D2 | "la propuesta actual está optimizada para mobile. El objetivo es tomar esta base como referencia y adaptarla al formato desktop/web" | Layout desktop propio (no mobile estirado); breakpoints definidos y documentados; sin scroll horizontal | `[x]` [ADR-0005](./decisions/ADR-0005-layout-desktop.md). Lista en grilla de 1/2/3 columnas según ancho; detalle en dos columnas desde 1024px con la imagen al costado; navegación arriba compartiendo fila con el buscador en vez de barra inferior de pulgar. **El Figma no tiene pantallas de desktop**, así que es decisión propia y no traducción |
...
| E2 | "Buena implementación de los elementos UI tanto a nivel de código como visualmente" | Componentes de presentación reutilizables, sin lógica de datos adentro; estados hover/focus/disabled/vacío/error cubiertos | `[x]` Siete componentes en `components/ui/`, todos de presentación pura y **verificado por lint** (ver E3): `PokemonCard`, `TypeChip`, `FavoriteStar`, `SearchInput`, `EmptyState`, `AppNav`, `AppIcon`. Estados cubiertos: carga, error con reintento, dos vacíos distintos —búsqueda sin resultados y favoritos sin marcar—, y `:focus-visible` en todo control interactivo. Accesibilidad: `aria-pressed` en los toggles, `aria-posinset`/`aria-setsize` en la lista virtualizada, `role="status"` para lo que cambia sin recargar. **Pendiente conocido:** navegación por teclado sobre nodos reciclados |
...
| E5 | "Puntos extras si implementas unit test (no excluyente)" | Tests de store (favoritos), del cliente de API (mockeado) y de componentes clave; corren en CI | `[x]` **109 tests** en 9 archivos: los tres stores, el cliente de API mockeado, los cuatro composables y las dos views. **Corriendo en CI y verificado en verde** (GitHub Actions, 48 s): `type-check` + `lint:check` + tests + build, ningún paso con `continue-on-error` |
