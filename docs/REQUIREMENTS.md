...
| F4 | "`GET /api/v2/pokemon/${name}`" para info específica | Detalle se pide solo al abrir un Pokémon; resultado cacheado (no re-pide al reabrir) | `[~]` `getDetail()` cachea por nombre en un `Map` y deduplica requests concurrentes; `toPokemon()` convierte a kg/m y ordena los tipos por `slot`. Verificado contra la API real y con tests que cuentan llamadas al reabrir. **Falta verificar en Network con la app corriendo** |
